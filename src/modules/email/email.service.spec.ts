...
describe("EmailService", () => {
  ...
  it("issues a signup token only after a valid email code", async () => {
    ...
    const repository = {
      latestVerification: jest.fn().mockResolvedValue({ id: "verification", codeHash, expiresAt: new Date(Date.now() + 60_000), verifiedAt: null, attemptCount: 0 }),
    ...
    await expect(service.verifySignupCode("USER@example.com", "123456")).resolves.toEqual({ emailVerificationToken: expect.any(String) });
    expect(repository.createSignupToken).toHaveBeenCalledWith(expect.any(String), "user@example.com", "verification", expect.any(Date));
  });
